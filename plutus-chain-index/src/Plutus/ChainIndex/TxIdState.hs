...
import           Plutus.ChainIndex.Types     (BlockNumber (..), Depth (..), Tip (..), TxStatus (..), TxValidity (..))
...
rollback targetTip idx@(viewTip -> currentTip)
    ...
    | otherwise = do
        ...
        case tip (measure before) of
            ...
            oldTip | oldTip == targetTip -> let x = _usTxUtxoData (measure deleted)
                                                newTxIdState = TxIdState
                                                                  { txnsConfirmed = mempty
                                                                  , txnsDeleted = const 1 <$> txnsConfirmed x
                                                                  }
                                                newUtxoState = UtxoState newTxIdState targetTip
